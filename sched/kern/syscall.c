...
// Recibe el id de un env
// Devuelve la prioridad de un env si existe
// Devuelve -1 en caso contrario o si falla envid2env()
static int
...
static int
sys_env_lower_priority(envid_t envid)
{
	struct Env *e;
	if (envid2env(envid, &e, 1) < 0) {
		return -1;
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	case SYS_env_lower_priority:
		return sys_env_lower_priority(a1);
	default:
		return -E_INVAL;
	}
}
